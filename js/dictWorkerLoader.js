importScripts('/js/dictWorker.js');

var app = Elm.DictWorker.init();
...
app.ports.loadFromIndexedDb.subscribe(function(key) {
    ...
    var request = db.transaction(['japDict'])
        .objectStore('japDict')
        .index('filename')
        .get(key);
...
var request = self.indexedDB.open("japDictDB", 2);
...
request.onupgradeneeded = function(event) {
    ...
    if (!db.objectStoreNames.contains('japDict')) {
        objectStore = db.createObjectStore('japDict', {
            autoIncrement: true
        });
...
function addToDatabase(data) {
    ...
    var request = db.transaction(['japDict'], 'readwrite')
        .objectStore('japDict')
        .add({
